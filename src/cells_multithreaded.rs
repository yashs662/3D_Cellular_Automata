...
fn tick_cell(
    ...
    // generate a random number if the random number is bigger than 0.9 then spawn a uniform cube
    let random_number = rand::random::<f32>();
    if random_number > 0.99 {
        println!("random number: {}", random_number);
        utils::spawn_uniform_cube(&mut cells.states.write().unwrap(), &rule, 3);
    }
    ...
    if keyboard_input.just_pressed(KeyCode::Key2) {
        println!("unform cube size 2");
        utils::spawn_uniform_cube(&mut cells.states.write().unwrap(), &rule, 2);
        cells.ready();
    }
    if keyboard_input.just_pressed(KeyCode::Key3) {
        println!("unform cube size 3");
        utils::spawn_uniform_cube(&mut cells.states.write().unwrap(), &rule, 3);
        cells.ready();
    }
    if keyboard_input.just_pressed(KeyCode::Key4) {
        println!("unform cube size 4");
        utils::spawn_uniform_cube(&mut cells.states.write().unwrap(), &rule, 4);
        cells.ready();
    }
    if keyboard_input.just_pressed(KeyCode::Key5) {
        println!("unform cube size 5");
        utils::spawn_uniform_cube(&mut cells.states.write().unwrap(), &rule, 5);
        cells.ready();
    }
    if keyboard_input.just_pressed(KeyCode::Key6) {
        println!("unform cube size 6");
        utils::spawn_uniform_cube(&mut cells.states.write().unwrap(), &rule, 6);
        cells.ready();
    }
...
impl Plugin for CellsMultithreadedPlugin {
    fn build(&self, app: &mut bevy::prelude::App) {
        ...
        app.insert_resource(cells_singlethreaded)
            ...
            .add_system(spawn_uniform_cube)
            .add_system(clear_screen)
            .add_system(spawn_random)

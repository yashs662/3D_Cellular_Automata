...
pub fn spawn_uniform_cube(states: &mut HashMap<IVec3, CellState>, rule: &Rule, spawn_size: i32) {
    for x in -spawn_size..=spawn_size {
        for y in -spawn_size..=spawn_size {
            for z in -spawn_size..=spawn_size {
                let pos = ivec3(x, y, z);
                let dist = dist_to_center(pos, rule);
                if x != -spawn_size && x != spawn_size && y != -spawn_size && y != spawn_size && z != -spawn_size && z != spawn_size {
                    states.insert(pos, CellState::new(rule.states, 0, dist));
                }
            }
        }
...
pub fn clear_states(states: &mut HashMap<IVec3, CellState>) {
    states.clear();
}
